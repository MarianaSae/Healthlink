...
/* ============================================================
   ...
   Endpoints:
     ...
     POST api.php?resource=citas  {doctor_id, paciente_nombre, fecha, hora}
   ============================================================ */
...
switch ($resource) {
    ...
    case 'citas':
        if ($method === 'POST') {
            ...
            $db   = getDB();
            $stmt = $db->prepare(
                "INSERT INTO citas (doctor_id, paciente_nombre, paciente_email, fecha, hora, motivo, estado, creado_en)
                 VALUES (?, ?, ?, ?, ?, ?, 'pendiente', NOW())"
            );
            $stmt->bind_param('isssss', $doctor_id, $paciente_nombre, $paciente_email, $fecha, $hora, $motivo);
            $stmt->execute();
